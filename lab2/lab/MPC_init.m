...
%% For real-time implementaiton
global W_delta;
W_delta = 2 * (Hy'*Qy*Hy +R);
size_W_delta = size(W_delta);
W_delta = [W_delta, zeros(size_W_delta(1), 1); zeros(1, size_W_delta(2)), 0];
global A_iq;
A_iq = W;
size_A_iq = size(A_iq);
A_iq = [A_iq, -ones(size_A_iq(1), 1); zeros(1, size_A_iq(2)), -1];
